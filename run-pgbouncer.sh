...
git clone --depth=1 https://github.com/HaithamSaqr/odoo-18-docker-compose-pgbouncer $DESTINATION
...
# Set ports in docker-compose-pgbouncer.yml
# Update docker-compose-pgbouncer configuration
if [[ "$OSTYPE" == "darwin"* ]]; then
  # macOS sed syntax
  sed -i '' 's/10018/'$PORT'/g' $DESTINATION/docker-compose-pgbouncer.yml
  sed -i '' 's/20018/'$CHAT'/g' $DESTINATION/docker-compose-pgbouncer.yml
else
  # Linux sed syntax
  sed -i 's/10018/'$PORT'/g' $DESTINATION/docker-compose-pgbouncer.yml
  sed -i 's/20018/'$CHAT'/g' $DESTINATION/docker-compose-pgbouncer.yml
...
# Run Odoo
docker-compose-pgbouncer -f $DESTINATION/docker-compose-pgbouncer.yml up -d
